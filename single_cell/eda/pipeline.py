...
def qc(
    ...
    *,
    ...
    if labels is None:
        ...
        return X[good_cells][:, good_genes].copy().astype("int32"), labels[good_cells]
...
def labels_to_ids(series: pd.Series) -> list:
    ...
    return np.array(list(map(lambda label: labels_to_ids_dict[label], series))), labels_to_ids_dict


def plot(
    u, labels=None, colorscale="Viridis",
    backend="plotly",
        ...
        if u.shape[1] == 2:
            fig = go.Figure(
                data=go.Scattergl(
                    ...
                    # z=u[:, 2],
                    mode="markers",
                    marker=dict(size=2, color=labels, colorscale=colorscale, opacity=0.9),
                )
            )
        elif u.shape[1] == 3:
            fig = go.Figure(
...
def magic_clustering(X, X_pca, pca_k, magic_k):
    ...
    print("UMAPing")
    magic_embedding, X_magic_umap = umap(X_magic_pca, n_components=3, n_neighbors=30, min_dist=0.0)
...
def test_dl(model, X: np.ndarray, device: str, apply_pca: bool, pca_k: int):
    ...
    if apply_pca:
        ...
        print("UMAPing...")
        magic_embedding, X_umap = umap(X_pred_pca, n_components=3, n_neighbors=10, min_dist=0.1)
    else:
        print('UMAPing...')
        magic_embedding, X_umap = umap(X_pred, n_components=3, n_neighbors=10, min_dist=0.1)
