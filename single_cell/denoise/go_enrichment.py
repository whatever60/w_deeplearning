...
gene_df = pd.read_csv(DATA_DIR_NCBI + 'human.tsv', sep='\t', dtype={'chromosome': 'str'}).drop_duplicates(subset='Symbol')[['Symbol', 'GeneID']]
...
go_labels[(gene2go_df_good.GO_ID.map(go2index), gene2go_df_good.GeneID.map(geneid2index))] += 1
...
def rank_test(preds: np.ndarray, targets: np.ndarray) -> np.ndarray:
    res = zip(*[stats.spearmanr(pred, target) for pred in tqdm(preds) for target in targets])
...
def multilabel_classification(preds: torch.Tensor, targets: torch.Tensor) -> np.ndarray:
    ...
    sim = np.array([loss(pred.unsqueeze(0), target.unsqueeze(0)) for pred in tqdm(preds) for target in targets]).reshape(preds.shape[0], targets.shape[0])
    return sim
