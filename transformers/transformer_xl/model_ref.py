...
class PositionalEmbedding(nn.Module):
    ...
    def forward(
        self,
        positions: torch.LongTensor,  # (seq, )
    ):
        # outer product
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        self,
        input_: torch.FloatTensor,  # (cur_seq, b, d_in)
        ...
        input_with_memory = torch.cat(
            [memory, input_], dim=0
        )  # concatenate recurrent memory
        ...
        k_tfmd, v_tfmd = torch.chunk(
            self.linear_kv(input_with_memory), 2, dim=-1
        )  # (cs + ps, b, H * d)
        ...
        content_attn = torch.einsum(
            ...
            (
                (
                    q_tfmd.view(cur_seq, bs, H, d) + u  # (a)
                ),  # (c): u represents the global (independent of the query)
                # bias towards certain key/values = words
                # Note: maybe this could be a per-attention head parameter?
                k_tfmd.view(
                    cur_seq + prev_seq, bs, H, d
                ),  # There is no positional information to be found here
            ),
        )  # (cs, cs + ps, b, H)
        ...
        position_attn = torch.einsum(
            ...
            (
                (
                    q_tfmd.view(cur_seq, bs, H, d) + v  # (b)
                ),  # (d): v represents the global (independent of the query)
                # bias towards certain positions
                p_tfmd.view(
                    cur_seq + prev_seq, H, d
                )  # Notice there is not content information
...
class PositionwiseFF(nn.Module):
    ...
    def forward(
        self,
        input_: torch.FloatTensor,  # (cur_seq, bs, d_input)
...
class DecoderBlock(nn.Module):
    def __init__(
        self, n_heads, d_input, d_head_inner, d_ff_inner, dropout, dropouta=0.0
    ):
        super().__init__()
        self.mha = MultiHeadAttention(
            d_input, d_head_inner, n_heads=n_heads, dropout=dropout, dropouta=dropouta
        )
...
class TransformerXL(nn.Module):
    ...
    def update_memory(
        self,
        # memory: List[torch.FloatTensor],
        hidden_states: List[torch.FloatTensor],
    ):
        assert len(hidden_states) == len(self.memory)
        mem_len, seq_len = self.memory[0].size(0), hidden_states[0].size(0)
    ...
    def forward(
        self,
        idxs: torch.LongTensor,  # (cs, bs)
        # target: torch.LongTensor,  # (cs, bs)
        # memory: Optional[List[torch.FloatTensor]] = None,
    ) -> Dict[str, torch.Tensor]:
        if self.memory is None:
            self.init_memory(idxs.device)
        assert len(self.memory) == len(self.layers) + 1
        cur_seq, bs = idxs.size()
        prev_seq = self.memory[0].size(0)
        ...
        pos_idxs = torch.arange(cur_seq + prev_seq - 1, -1, -1.0, dtype=torch.float).to(
            word_embs.device
        )
        ...
        layer_out = word_embs
        for mem, layer in zip(self.memory, self.layers):
            layer_out = layer(
