...
def _get_cv_home():
    cv_home = os.path.expanduser(
        os.getenv(
            ENV_MMCV_HOME,
            os.path.join(os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), "cv"),
        )
    )

    mkdir_or_exist(cv_home)
    return cv_home
...
def get_external_models():
    cv_home = _get_cv_home()
    default_json_path = osp.join(cv.__path__[0], "model_zoo/open_mmlab.json")
    default_urls = load_file(default_json_path)
    assert isinstance(default_urls, dict)
    external_json_path = osp.join(cv_home, "open_mmlab.json")
...
def get_mmcls_models():
    mmcls_json_path = osp.join(cv.__path__[0], "model_zoo/mmcls.json")
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(cv.__path__[0], "model_zoo/deprecated.json")
...
class CheckpointLoader:
    ...
    def load_checkpoint(cls, filename, map_location=None, logger=None):
        ...
        cv.print_log(f"Use {class_name} loader", logger)
...
def load_from_openmmlab(filename, map_location=None):
    ...
    if model_url.startswith(("http://", "https://")):
        ...
        filename = osp.join(_get_cv_home(), model_url)
        if not osp.isfile(filename):
            raise IOError(f"{filename} is not a checkpoint file")
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
...
def save_checkpoint(model, filename, optimizer=None, meta=None):
    ...
    meta.update(cv_version=cv.__version__, time=time.asctime())
    ...
    if filename.startswith("pavi://"):
        ...
        cv.mkdir_or_exist(osp.dirname(filename))
